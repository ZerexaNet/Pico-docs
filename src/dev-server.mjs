...
export async function watchAndServe(rootDir, options = {}) {
  ...
  const buildResult = await buildSite(rootDir);
  let outDir = buildResult.outDir;
  let i18nProxyConfig = normalizeI18nProxyConfig(buildResult.config?.i18n);
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      const translateProxyPath = normalizeLocalEndpointPath(i18nProxyConfig.endpoint);
      if (translateProxyPath && requestUrl.pathname === translateProxyPath) {
        await handleTranslateProxy(req, res, i18nProxyConfig);
        return;
      }
    ...
    watchPath(watchTarget, () => {
      ...
      rebuildTimer = setTimeout(async () => {
        ...
        try {
          ...
          i18nProxyConfig = normalizeI18nProxyConfig(result.config?.i18n);
...
async function handleTranslateProxy(req, res, i18nProxyConfig) {
  setCorsHeaders(res);
  ...
  const result = await proxyTranslateRequest(parsedBody, {
    endpoint: i18nProxyConfig.upstreamEndpoint
  });
...
function normalizeI18nProxyConfig(i18n) {
  return {
    endpoint: i18n?.endpoint,
    upstreamEndpoint: normalizeUpstreamEndpoint(i18n?.upstreamEndpoint)
  };
}

function normalizeLocalEndpointPath(endpoint) {
  if (typeof endpoint !== "string" || !endpoint.trim()) return "/api/translate";
  const trimmed = endpoint.trim();
  if (/^https?:\/\//iu.test(trimmed)) return null;

  const rawPath = trimmed.split("#")[0].split("?")[0];
  if (!rawPath) return "/api/translate";

  const normalized = rawPath.startsWith("/") ? rawPath : `/${rawPath}`;
  return normalized.replace(/\/+$/u, "") || "/";
}

function normalizeUpstreamEndpoint(endpoint) {
